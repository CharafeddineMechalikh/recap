...
public class LinknovateValidationLauncher {
	...
	public static void main(String[] args) throws IOException {
		...
		// write to disk the Proto model
		String simFilename = "ReadyModels/ElasticSearchExperiment.sim";
		// Write the to disk.
		FileOutputStream protoOutput = new FileOutputStream(simFilename);
		config.writeTo(protoOutput);
		protoOutput.close();
		
		// serialise to JSON
		String simFilenameJson = "ReadyModels/ElasticSearchExperiment.json";
		Printer protoPrinter = com.google.protobuf.util.JsonFormat.printer().preservingProtoFieldNames();
		Writer protpWr = new FileWriter(simFilenameJson);
		protpWr.flush();
		protoPrinter.appendTo(config, protpWr);
		protpWr.close();
