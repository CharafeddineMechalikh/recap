...
public class RecapSimPlusLauncher {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Option experimentModelFilename = new Option("e", "experiment",true, "Infrastructure model file path");
		//experimentModelFilename.setRequired(true);
		test.addOption(experimentModelFilename);
		...
		if (cmd.hasOption("help")) {
			...
			config = ExperimentHelpers.GenerateConfiguration("Test-Config", 2,rim,ram,rwm);//duration only will work if simulation runs longer
		...
		//####
		try {
			config = Experiment.parseFrom(new FileInputStream(cmd.getOptionValue("e")));
			System.out.println("Loaded "+ config.getName());
			System.out.println("Simulation duration: "+ config.getDuration());
		...
		if(config!=null){
			...
			System.out.println("Unable to load experiment. Check your inputs:");
			for(String arg : args){
